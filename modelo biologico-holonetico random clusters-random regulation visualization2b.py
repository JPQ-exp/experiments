...
class BioHoloneticModel:
    ...
    def compute_delta_state(self, external_force: np.ndarray) -> np.ndarray:
        ...
        for i in range(self.n_dimensions):
            ...
            if is_positive:
                selected_cluster = self.clusters_positive[selected_cluster_index, i]
                print(f"Random positive cluster selected for dimension {i}: {selected_cluster}")
            else:
                selected_cluster = self.clusters_negative[selected_cluster_index, i]
                print(f"Random negative cluster selected for dimension {i}: {selected_cluster}")

            # Add the selected cluster contribution
            cluster_contributions[i] += selected_cluster
